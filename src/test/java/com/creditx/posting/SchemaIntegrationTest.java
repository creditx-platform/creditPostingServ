...
public class SchemaIntegrationTest {
  ...
  void testFlywayAppliedSchema() {
    ...
    Integer processedEventsTableCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM user_tables WHERE table_name = 'CPS_PROCESSED_EVENTS'",
        Integer.class);
    ...
    Integer outboxEventsTableCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM user_tables WHERE table_name = 'CPS_OUTBOX_EVENTS'", Integer.class);
    ...
    Integer processedCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM CPS_PROCESSED_EVENTS WHERE EVENT_ID = ?", Integer.class,
        "test-event-123");
    ...
    Integer outboxCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM CPS_OUTBOX_EVENTS WHERE AGGREGATE_ID = ?", Integer.class, 12345L);
